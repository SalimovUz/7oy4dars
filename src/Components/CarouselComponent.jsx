...
function Pageone() {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-white relative border-b-[1px]">
      <div
        className={`absolute top-0 left-0 m-4 p-4 shadow-lg h-[40%] ${colors[index]}`}
      >
        <h1 className="text-8xl font-semibold text-black w-[700px] h-[50%]">
          {texts[index]}
        </h1>
      </div>
      <p className="text-black mt-40 w-[40%] mr-[30%]">
      ...
      <div className="absolute bottom-4 mb-[50px] right-4">
        <img
          src={images[index]}
          alt={`Slide ${index + 1}`}
          className="w-540 h-80 rounded-lg shadow-lg"
        />
      </div>
    </div>
  );
